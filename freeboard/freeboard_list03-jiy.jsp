...
<!--  페이징 처리 부분 추가 freeboard_list03.jsp -->  
...
<!-- JSP 코드 블락 : DB 의 레코드를 가져와서 루프 : 시작 -->
	<%
	...
	Vector keyid = new Vector();	// DB 의 id 컬럼의 값을 저장하는 vector
	...
	String em = null; 	//DB에서 mail 주소를 가져와서 처리하는 변수
		...
		// rs 의 값을 Vector에 저장 : 페이징 처리하기 위함
		
		if(!(rs.next())){ // rs의 값이 존재하지 않을때
			out.println("게시판에 올라온 글이 없습니다.");
		}else{	// rs의 값이 존재할때 ( 게시판의 글이 존재 할 때 )
			do {
		...
		// endrow 가 totalrows 보다 크면 totalrow -1로 처리해야 함
		if (endrow >= totalrows) {
			endrow = totalrows -1;
		}
		...
		out.println("<p/> 시작하는 페이지  (startrow  : ) :"+ startrow  + "<p/>");
		out.println("<p/> 끝나는 페이지 (endrow : ) :"+ endrow + "<p/>");
		*/
...
<%
	// [처음][이전]
	if( wheregroup > 1 ){	// 현재 나의 그룹이 1 이상일때 처음과 
	...
	if (name.size() !=0 ){		// name.size() :  총 레코드의 갯수가 0이 아니라면
		for(int jj = startpage; jj <= endpage; jj++){
			if(jj == where){	// jj가 자신의 페이지 번호라면 링크 없이 출력
				out.println("[" + jj +"]");
