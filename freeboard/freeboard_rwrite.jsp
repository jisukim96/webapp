...
<%
	...
	int rnum = 0;			//freeborad 테이블의 replaynum 컬럼의 값을 저장
	
	int id = Integer.parseInt(request.getParameter("id")); //<- 가지고 답변글 생성
	
	sql = "select * from freeboard where id =" + id;
	stmt = conn.createStatement();
	rs = stmt.executeQuery(sql);		//select한 결과 레코드 가지고 있음
	
	if(!(rs.next())){
		out.println("해당 내용이 없습니다.");
	}else { //rs의 값이 존재
		cont = " > " + rs.getString("content");		//DB의 내용을 가져와서 글 앞에 화살표 ( > ) 표시
