...
<body>
	...
	//모두 String 형식으로 넘어옴 -> Integer.ParseInt();
	...
	*/
	...
	//PreparedStatement 객체를 사용해서 처리, 변수 들어가는 자리를 ?로 처리함.
			// 매번 컴파일 하지 않기 때문에 처리속도가 빠름
	sql = "select * from freeboard where id = ?";
	pstmt = conn.prepareStatement(sql);			//pstmt 객체 생성시 sql 구문을 넣는다
	pstmt.setInt(1, id);				// 1 첫번째 물음표 , 들어갈 변수
	rs = pstmt.executeQuery();			// select 쿼리인 경우 -> executeQuery()
										// insert/update/delete -> executeUpdate() 를 실행
	...
	<table width='600' cellspacing='0' cellpadding='2' align='center'>
		...
		<tr align='center' bgcolor='#DFEDFF'>
			<td class='button' bgcolor='#DFEDFF'>
				<div align='left'>
					<font size='4'>글제목 : <%= rs.getString("subject") %></font>
				</div>
			</td>
		</tr>
		...
		<tr align='center'>
			<td style='padding: 10 0 0 0'>
				<div align='left'>
					<br> <font size='3' color='#333333'><pre><%= rs.getString("content") %></pre></font>
				</div> <br>
			</td>
		</tr>
		<tr align='center'>
	...
	<table width="600" border="0" cellpadding="0" cellspacing="5">
		<tr>
			<td align="right" width="450"><A href="freeboard_list03.jsp?go=<%=request.getParameter("page")%>">
			...
			<td width="70" align="right"><A href="freeboard_upd.jsp?id=<%= id %>&page=<%= request.getParameter("page")%>">
			<img src="image/edit.jpg" border=0></A></td>
			<td width="70" align="right"><A href="freeboard_del.jsp?id=<%= id %>&page=<%= request.getParameter("page")%>">
...
<%
	...
	//Statement 객체의 select 쿼리인 경우 executeQuery() 를 실행 , ResultSet 객체로 리턴
	stmt = conn.createStatement();
